...
import { ArrowLeft, Plus, Trash2, Save, MapPin, User, FileText, Package } from "lucide-react";
...
export default function EditOrderPage() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const [order, custRes, prods] = await Promise.all([
          orderApi.getById(orderId),
          customerApi.getAll(0, 1000),
          productApi.getAvailable(),
        ]);

        if (order.status !== "PENDING") {
          setLoadError("Only PENDING orders can be edited.");
        ...
        setShippingCity(order.shippingCity);
        setNotes(order.notes || "");
        setItems(
          order.items.map((item) => ({
            productId: String(item.productId),
            quantity: String(item.quantity),
            unitPriceAtOrder: String(item.unitPriceAtOrder),
  ...
  if (pageLoading) return <LoadingSpinner message="Loading order..." />;
  if (loadError) return <ErrorMessage message={loadError} />;

  return (
    <div style={{ maxWidth: 900 }}>
      <Link href={`/dashboard/orders/${orderId}`} className="back-link mb-3 d-inline-flex">
        <ArrowLeft size={16} />
        Back to Order
      </Link>

      <div className="mb-4">
        <h1 className="fw-bold text-navy mb-1" style={{ fontSize: "1.5rem" }}>Edit Order</h1>
        <p className="text-muted-brand mb-0" style={{ fontSize: ".875rem" }}>Update the order details below</p>
      </div>
          ...
          <div className="card-brand-body">
            <div className="row g-3">
              <div className="col-12 col-sm-6">
                ...
                <select required value={customerId} disabled
                  className="input-brand" style={{ opacity: 0.7 }}>
                ...
                <small className="text-muted-brand">Customer cannot be changed</small>
              </div>
            </div>
          </div>
        </div>
